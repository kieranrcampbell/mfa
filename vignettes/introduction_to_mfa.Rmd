...
To extract these we can access the trace slot of the `m` object:

```{r posterior-mean-chi}
posterior_mean_chi <- colMeans(m$traces$chi_trace)
```

and we can sort this into a data frame if necessary:

```{r sort-to-df}
data_frame(feature = m[['feature_names']], chi = posterior_mean_chi) %>% 
  head()
```
